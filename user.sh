...
curl -o /tmp/user.zip https://roboshop-artifacts.s3.amazonaws.com/user-v3.zip &>>Logs_file.  #Downloads the catalogue app zip file.
...
unzip /tmp/user.zip &>>Logs_file               #Unzips the new code.
VALIDATE $? "Unzip user"

npm install &>>Logs_file                            #Installs required Node.js packages from package.json.
VALIDATE $? "Installing Dependencies"

cp $SCRIPT_DIR/user_service /etc/systemd/system/user.service  #Copies the service file to systemd.
